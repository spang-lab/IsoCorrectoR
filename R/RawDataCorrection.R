...
RawDataCorrection <- function(file_num, MoleculeData, MoleculeName, CombFragmentProb, MoleculeNo, SampleNo, roundDigits, logEnvironment) {
    ...
    MoleculeUncorr <- file_num[TransitionLocation, SampleNo]
    
    ProbMatrix <- CombFragmentProb
    
    # In the following, the linear equation system is solved with linear inequality constraints: ConstraintMatrix*SolutionVector <= ConstraintVector. The
    # ConstraintVector is 0, the ConstraintMatrix is -1 at each ConstraintMatrix(TransitionNo, TransitionNo) position. It is thereby assured that the
    # solutions cannot be < 0. If a solution is < 0, ConstraintMatrix*SolutionVector becomes positive and the constraint ConstraintMatrix*SolutionVector <=
    ...
    # Check the vector of uncorrected values for values that are not a number (e.g. NA) and thus missing.
    CheckForNaN <- is.na(MoleculeUncorr)
    
    # If there are no missing values, perform the correction for the current vector of uncorrected values MoleculeUncorr. This is done by numerically solving
    # a linear equation system where each uncorrected value is a linear combination of unknown corrected values with their probabilites of contributing to the
    # uncorrected value as the constants (from ProbMatrix).  ||ProbMatrix*CorrTransitionsPlus - MoleculeUncorr||^2 is minimized in a linear least squares
    # approach with the constraint that the corrected values (CorrTransitionsPlus) must not be < 0. In addition to CorrTransitionsPlus, also the residuals of
    # the solving process are given (CorrResiduals).
    
    if (sum(CheckForNaN) == 0) {
        
        CorrTransitionsPlus <- pracma::lsqlincon(C = ProbMatrix, d = as.numeric(MoleculeUncorr), A = ConstraintMatrix, b = ConstraintVector)
        ManResiduals <- ProbMatrix %*% CorrTransitionsPlus - MoleculeUncorr
        
    } else {
        
        notification <- paste0("In measurement data file: Meausrement data of molecule ", MoleculeName, " in sample ", colnames(file_num)[SampleNo], " contains NA values. The correction performed may be less acurate.", 
        ...
        # Get indices of missing values. The rows with these indices removed from MoleculeUncorr and the ConstraintVector. Row and column with these indices are
        # removed from ProbMatrix and ConstraintMatrix. This way, the missing values are completely removed from the linear equation system.
        
        # get index of NaN values
        nan.index <- as.numeric(which(CheckForNaN))
        
        # remove respective rows/columns
        MoleculeUncorr.nan <- MoleculeUncorr[-nan.index]
        ...
        if (length(nan.index) < NumberTransitions) {
            CorrTransitionsPlusNaN <- pracma::lsqlincon(C = ProbMatrix.nan, d = as.numeric(MoleculeUncorr.nan), A = ConstraintMatrix.nan, b = ConstraintVector.nan)
            ManResidualsNaN <- ProbMatrix.nan %*% CorrTransitionsPlusNaN - MoleculeUncorr.nan
        } else {
            notification <- paste0("In measurement data file: All measurements are NA for molecule ", MoleculeName, " in sample ", colnames(file_num)[SampleNo], 
                ".")
            errorHandler(notification, logEnvironment, "warning")
        }
    ...
    # The corrected values in CorrTransitionsPlus are values that correspond to the full integral of the isotopologue abundance distribution of a given
    # labeled species. To get the value (CorrTransitions) that corresponds to the species in the distribution that contains no natural isotopes of higher
    # mass, the corresponding CorrTransitionsPlus value has to be multiplied with the probability that the species contains no isotopes of higher mass due to
    # natural abundance
    ...
    for (TransitionNo in seq_len(NumberTransitions)) {
        ...
        if (is.na(CorrTransitionsPlus[TransitionNo]) == FALSE) {
            ...
            CorrTransitions[TransitionNo] <- CorrTransitionsPlus[TransitionNo] * CombFragmentProb[TransitionNo, TransitionNo]
    ...
    returnList <- list(CorrectedMonoisotopic = CorrTransitions, Corrected = CorrTransitionsPlus, CorrectedMonoisotopicFractions = CorrTransitionsFractions, 
